...
// =============================================================================
// TODO 1: Tune the PID gains.
//   The values below are STUBS (1.0). Cyclops will not hold yaw or depth
//   until you replace them with values you tune yourself.
//   Rough starting ranges (see lab slides):
//     YAW_ANGLE_GAIN_P : ~10  -- 100
//     YAW_GAIN_P/I/D   : ~10  -- 200  /  ~1 -- 10  /  ~1 -- 50
//     DEPTH_GAIN_P/I/D : ~1k  -- 10k  /  ~10 -- 100 / ~100 -- 1000
//   Recommended tuning order: increase P until you see oscillation, then add
//   D to damp it, then add a small I to remove steady-state error.
// =============================================================================
#define YAW_ANGLE_GAIN_P 1.0   // <-- TODO: tune
#define YAW_GAIN_P 1.0         // <-- TODO: tune
#define YAW_GAIN_I 1.0         // <-- TODO: tune
#define YAW_GAIN_D 1.0         // <-- TODO: tune

#define DEPTH_GAIN_P 1.0       // <-- TODO: tune
#define DEPTH_GAIN_I 1.0       // <-- TODO: tune
#define DEPTH_GAIN_D 1.0       // <-- TODO: tune

// X / Y position-hold gains -- only used by the advanced pos_xy_control mode.
// Leave these alone for the in-class assignment.
...
ros::Publisher *pub_thruster0_ptr; // heave-up, right
ros::Publisher *pub_thruster1_ptr; // heave-up, left

ros::Publisher *pub_thruster2_ptr; // surge, right
ros::Publisher *pub_thruster3_ptr; // surge, left

ros::Publisher *pub_thruster4_ptr; // rear-right yaw, counter-clockwise
ros::Publisher *pub_thruster5_ptr; // front-right yaw, clockwise
ros::Publisher *pub_thruster6_ptr; // front-left yaw, counter-clockwise
ros::Publisher *pub_thruster7_ptr; // rear-left yaw, clockwise
...
void msgCallbackDepth(const sensor_msgs::FluidPressure::ConstPtr &msg)
{
    ...
    // =========================================================================
    // TODO 2a: Implement the depth PID control law.
    //   Inputs (already prepared for you):
    //     - target_depth        : desired depth (m)
    //     - current_depth       : depth from pressure sensor (m)
    //     - error_pid_depth1    : previous step's error (used for D term)
    //     - DEPTH_T             : loop period (s)
    //     - DEPTH_GAIN_P/I/D    : your tuned gains from TODO 1
    //   Outputs you must compute (in this exact order):
    //     1) error_pid_depth = target_depth - current_depth
    //     2) P_depth         = error_pid_depth * DEPTH_GAIN_P
    //     3) D_depth         = ((error_pid_depth - error_pid_depth1) / DEPTH_T) * DEPTH_GAIN_D
    //     4) I_depth        += error_pid_depth * DEPTH_T * DEPTH_GAIN_I
    //     5) I_depth         = constrain(I_depth, -500, 500)   // anti-windup
    //     6) PID_depth       = P_depth + D_depth + I_depth
    // =========================================================================
    error_pid_depth = target_depth - current_depth;
    // P_depth   = ???
    // D_depth   = ???
    // I_depth  += ???
    // I_depth   = constrain(I_depth, -500, 500);
    // PID_depth = ???
...
void msgCallbackIMU(const sensor_msgs::Imu::ConstPtr &msg)
{
    ...
    // Yaw wrap-around handling (already done for you).
    ...
    // =========================================================================
    // TODO 2b: Implement the yaw PID control law (cascaded P-PID).
    //   Cascade structure:
    //     OUTER P loop on angle error  ->  feeds into INNER PID loop on rate.
    //   Inputs:
    //     - target_yaw            : desired yaw (rad)
    //     - current_yaw           : measured yaw (rad, with wrap fix)
    //     - current_acc_yaw       : measured yaw rate (rad/s, from IMU)
    //     - error_pid_yaw1        : previous step's inner error
    //     - YAW_T                 : loop period (s)
    //     - YAW_ANGLE_GAIN_P      : outer P gain (angle -> rate setpoint)
    //     - YAW_GAIN_P/I/D        : inner PID gains (rate)
    //   Steps to implement (in order):
    //     1) error_yaw       = target_yaw - current_yaw                     // angle err
    //     2) P_angle_pid_yaw = YAW_ANGLE_GAIN_P * error_yaw                 // outer P -> rate setpoint
    //     3) error_pid_yaw   = P_angle_pid_yaw - current_acc_yaw            // rate err
    //     4) P_yaw           = error_pid_yaw * YAW_GAIN_P
    //     5) D_yaw           = ((error_pid_yaw - error_pid_yaw1) / YAW_T) * YAW_GAIN_D
    //     6) I_yaw          += error_pid_yaw * YAW_T * YAW_GAIN_I
    //     7) I_yaw           = constrain(I_yaw, -500.0, 500.0)
    //     8) PID_yaw         = P_yaw + D_yaw + I_yaw
    // =========================================================================
    // error_yaw       = ???
    // P_angle_pid_yaw = ???
    // error_pid_yaw   = ???
    ...
    // D_yaw           = ???
    // I_yaw          += ???
    // I_yaw           = constrain(I_yaw, -500.0, 500.0);
    // PID_yaw         = ???


    // X/Y position-hold PID -- already provided. Used only when pos_xy_control == 1.
    ...
    // =========================================================================
    // TODO 3: Implement thruster mixing for every motion state.
    //   The 'stop' branch (move_command == 0) is given as a worked example.
    //   You must implement: forward (1), backward (2), sway-left (3), sway-right (4).
    //
    //   Hints (see lab slides for the exact signs):
    //     * Yaw PID always rides on thrusters 4..7 with +/- THROTTLE bias.
    //     * Forward/backward = +/- velocity offset on thrusters 2 and 3.
    //     * Sway = velocity offset biased on thrusters 4..7 (with opposing signs
    //       on the front pair vs rear pair so the body translates instead of yawing).
    //   Skip the pos_xy_control == 1 sub-branches for the in-class assignment;
    //   they're optional advanced work.
    // =========================================================================
    if (move_command == 0) // stop -- worked example
    {
    ...
    else if (move_command == 1) // forward -- TODO 3
    {
        // TODO: Apply yaw PID to thrusters 4..7 (same as 'stop'),
        //       and add -velocity offset to thrusters 2 and 3.
    }
    else if (move_command == 2) // backward -- TODO 3
    {
        // TODO: Same yaw PID layout, +velocity offset on thrusters 2 and 3.
    }
    else if (move_command == 3) // sway-left -- TODO 3
    {
        // TODO: Bias thrusters 4..7 to translate left;
        //       thruster_2 and thruster_3 stay at 0.
    }
    else if (move_command == 4) // sway-right -- TODO 3
    {
        // TODO: Bias thrusters 4..7 to translate right;
        //       thruster_2 and thruster_3 stay at 0.
    }
